...
var samplerApp = function(options) {
  ...
  function recalc() {
    ...
    const queriedDatumPerSourcePerHourCount = configurationNumber('queriedDatumPerSourcePerHourCount');

    const datumPerHourCount = configurationNumber('datumPerHourCount', nodeCount * sourcesPerNodeCount * datumPerSourcePerHourCount);
    const propertiesPerHourCount = configurationNumber('propertiesPerHourCount', datumPerHourCount * propertiesPerDatumCount);
    ...
    const datumQueriedPerMonth = sourcesPerNodeCount * queriedDatumPerSourcePerHourCount * hoursPerMonth;
    ...
    for ( let monthNum = 1, len = config.numMonths; monthNum <= len; monthNum += 1 ) {
      ...
      let monthCost = Number(Number(propInCostPerMonth + datumQueriedCostPerMonth + datumDaysStoredCost).toFixed(2));
  ...
  function setupSubscriptionRatesTable(table) {
    ...
    for ( let [key, tierData] of tiers ) {
      ...
      for ( let i = 0, len = tierData.length; i < len; i += 1 ) {
        ...
        rowData.set('rate', `${costFormat.format(tier.rate * millionsBase * 1000000)} / ${millionsBase} million`);
